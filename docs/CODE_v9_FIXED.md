# XAUUSD SMC Pro v9 (fixed) — โค้ดเต็มสำหรับคัดลอก

v9 ต้นฉบับ แก้เฉพาะบั๊ก 11 จุด **ไม่เพิ่มฟีเจอร์ ไม่เพิ่ม input สักช่อง**
ไม่ยึดตาม v9.1 / v9.4 / v9.5 / v10 — ทำจาก `XAUUSD_SMC_Pro_v9.pine` ตรง ๆ

รายละเอียดว่าแก้อะไรบ้าง และอะไรที่ตั้งใจไม่แก้ อ่านที่ [V9_FIXED.md](V9_FIXED.md)
...
//   FIX 7  บัฟเฟอร์ swing ถูกบวกหลังเช็คเพดาน → SL ไปจบที่เพดานพอดีโดยไม่ตั้งใจ
//   FIX 8  buyQualOK เป็นจริงเสมอ ไม่เคยกรองอะไร
//   FIX 9  เลข point คิดจาก 0.01 ตายตัว ผิดกับโบรกที่ทองมี 3 ทศนิยม
//   FIX 10 โหมด Limit นับ TP/SL ทั้งที่ออเดอร์อาจไม่เคยติด
//   FIX 11 array ของไม้/กล่อง FVG โตไม่จำกัด จนเกินลิมิต drawing แล้วเพี้ยน
//
// หมายเหตุ: คอมเมนต์ที่เขียนว่า "FIX #3" / "FIX #6" (มี # นำหน้า) เป็นของเดิม
//   ที่ติดมากับ v9 ไม่ใช่ของรอบนี้ ของรอบนี้ใช้ "FIX 1" ถึง "FIX 11" ไม่มี #
//
// ⚠️ สิ่งที่ "ไม่ได้แก้" โดยตั้งใจ เพราะเป็นการออกแบบ ไม่ใช่บั๊ก — อ่าน docs/V9_FIXED.md
//    เพดาน SL ยังบีบ SL ให้แคบแล้วเข้าไม้อยู่ดี / FVG ยังเป็นแค่กราฟิก
//    / Min Score 4 ยังผ่านได้โดยไม่มี BOS-CHoCH / เลข $ ยังคิดที่ 100 oz ต่อ lot
// ══════════════════════════════════════════════════════════════════════════
indicator("XAUUSD SMC Pro v9 (fixed)", overlay=true, max_labels_count=500, max_lines_count=500, max_boxes_count=500)
...
minScore    = input.int(4, "Min Score (แท่งปิด)", minval=2, maxval=7, group=grp_signal, tooltip="FIX 6: คะแนนเต็มคือ 7 แต่เดิมเลือกได้แค่ถึง 5\n\nระวัง: ที่ 4 คะแนน trendUp+rsiOK+trigger+volOK ครบพอดีโดยไม่ต้องมี BOS/CHoCH เลย ถ้าอยากบังคับให้ใช้โครงสร้าง SMC จริงต้องตั้ง 5 ขึ้นไป")
...
// MTF BIAS
// ══════════════════════════════════════════
// FIX 1 — คอมเมนต์เดิมเขียนว่า "no repaint" แต่ lookahead_off ไม่ได้แปลว่า
//   "ค่าที่ปิดแล้ว" มันคือค่าของแท่ง HTF ที่ *กำลังก่อตัวอยู่* บนกราฟ H1 ค่า
//   bias_d1 จึงเปลี่ยนไปมาได้ทั้งวัน แล้วประวัติไม่เหมือนเดิมเมื่อรีเฟรชกราฟ
//   สูตรที่ไม่ repaint จริงคือ lookahead_on + ดึงค่า [1] = แท่ง HTF ที่ปิดแล้วเท่านั้น
f_htfBias(_tf) =>
...
// FIX 2 — บนกราฟ H1 การขอ M15/M30 คือการขอ TF ที่ต่ำกว่ากราฟ ได้แค่ค่าสุดท้าย
//   ของ LTF ในแท่งนั้น ย้อนหลังกับ realtime ไม่ตรงกัน → ตัดออก แสดง n/a ในตาราง
//   ผลพลอยได้: allBull/allBear เดิมต้องครบทั้ง 5 TF รวม M15/M30 ที่เชื่อไม่ได้
//   ทำให้สัญญาณ realtime แทบไม่มีทางเกิดเมื่อเปิด rtNeedAlign
...
// FIX 3 — ถ้า TF ที่เลือกยืนยันไม่ได้สูงกว่ากราฟ ค่าที่ได้จะเท่ากับเทรนด์ของ
//   กราฟเองเป๊ะ ๆ = ฟิลเตอร์ไม่ได้กรองอะไร และคะแนนซ้ำกับ trendUp
//   ตอนนี้ใช้เทรนด์ของกราฟตรง ๆ ไปเลย ไม่แกล้งทำเป็นว่ามาจาก TF อื่น
localBias   = ema21 > ema50 ? 1 : ema21 < ema50 ? -1 : 0
...
// นับ align เฉพาะ TF ที่ใช้ได้จริง (ต่อจาก FIX 2)
...
if showFVG and barstate.isconfirmed
    ...
    if fvgMitig and array.size(bullBoxes) > 0
        for i = array.size(bullBoxes) - 1 to 0
            bx = array.get(bullBoxes, i)
            if low <= box.get_bottom(bx)
                box.delete(bx)
                array.remove(bullBoxes, i)
    if fvgMitig and array.size(bearBoxes) > 0
        for i = array.size(bearBoxes) - 1 to 0
            bx = array.get(bearBoxes, i)
            if high >= box.get_top(bx)
                box.delete(bx)
                array.remove(bearBoxes, i)
    // FIX 11 — array โตไม่จำกัด (โดยเฉพาะตอนปิด "ลบ FVG ที่ถูกเติมแล้ว" = ไม่มีอะไรลบเลย)
    //   พอกล่องรวมเกิน max_boxes_count TradingView จะลบกล่องเก่าทิ้งเอง
    //   แต่ array ยังถือ id เดิมอยู่ → box.get_bottom() ไปอ่านกล่องที่ไม่มีแล้ว
    //   จึงต้องลบพร้อมกันทั้งสองที่ (60 กล่องต่อฝั่ง = 120 ยังห่างจากลิมิต 500)
    while array.size(bullBoxes) > 60
        box.delete(array.shift(bullBoxes))
    while array.size(bearBoxes) > 60
...
trendUp = ema21 > ema50
trendDn = ema21 < ema50
// กันไล่ยอด/ก้น — Long ห้ามซื้อตอน RSI สูงเกิน, Short ห้ามขายตอน RSI ต่ำเกิน
// FIX 4 — เดิม `rsiVal < rsiOB and rsiVal < noChaseHi` โดยที่ noChaseHi(62) < rsiOB(70)
//   เสมอ เงื่อนไข rsiOB จึงไม่มีผลใด ๆ และเลข 35/65 เป็น hardcode ที่ไม่ผูกกับ
//   ช่อง RSI OS/OB เลย = ปรับสองช่องนั้นยังไงก็ไม่มีอะไรเปลี่ยน
rsiOK_buy  = rsiVal > rsiOS and rsiVal < math.min(rsiOB, noChaseHi)
rsiOK_sell = rsiVal < rsiOB and rsiVal > math.max(rsiOS, noChaseLo)
// FIX 5 — ฟีด XAUUSD หลายเจ้าไม่มี volume (na) ทำให้เงื่อนไขนี้เป็น false ตลอด
//   = เสียคะแนนฟรี 1 แต้มทุกแท่ง สัญญาณแทบไม่ผ่าน minScore
...
// FIX 7 — เดิมเช็ค `buySwingDist <= maxSLusd` ก่อน แล้วค่อยบวกบัฟเฟอร์ +0.1 ATR
//   ทีหลังแล้ว cap อีกที → ไม้ที่ swing อยู่ใกล้เพดานพอดีจะจบที่เพดานเป๊ะ ๆ
//   โดยไม่ได้ตั้งใจ ตอนนี้รวมบัฟเฟอร์เข้าไปก่อนแล้วค่อยตัดสิน
buySwingDist = not na(swL1) and swL1 < buyEntry ? buyEntry - swL1 + atrVal * 0.1 : na
...
sellSwingDist = not na(swH1) and swH1 > sellEntry ? swH1 - sellEntry + atrVal * 0.1 : na
...
// FIX 8 — ATR > 0 เสมอ เงื่อนไขเดิมจึงเป็นจริงตลอด ไม่เคยกรองอะไรเลย
//   เติมการกัน na ของ ATR ในแท่งแรก ๆ ของกราฟเข้าไป
buyQualOK  = not na(atrVal) and buySLdist > 0
sellQualOK = not na(atrVal) and sellSLdist > 0

// FIX 9 — เดิมหารด้วย 0.01 ตายตัว แต่ทองบางโบรก (เช่น OANDA) แสดง 3 ทศนิยม
//   = 1 point เท่ากับ 0.001 ป้ายจึงบอก "1200 point" ทั้งที่ของจริงคือ 12000
//   syminfo.mintick คือขนาดขั้นราคาที่เล็กที่สุดของสัญลักษณ์นั้น ๆ ตามจริง
//   (เลข $ ยังคิดที่ 100 oz ต่อ lot เหมือนเดิม — ถูกกับบัญชีมาตรฐานเท่านั้น)
ptSize     = syminfo.mintick
buyTP1Pts  = math.round(buySLdist * rr1 / ptSize)
buyTP2Pts  = math.round(buySLdist * rr2 / ptSize)
buySLPts   = math.round(buySLdist / ptSize)
...
sellTP1Pts = math.round(sellSLdist * rr1 / ptSize)
sellTP2Pts = math.round(sellSLdist * rr2 / ptSize)
sellSLPts  = math.round(sellSLdist / ptSize)
...
if array.size(trades) > 0
    for i = 0 to array.size(trades) - 1
        ...
        if tr.active
            ...
            // FIX 10 — โหมด Limit ป้ายบอกให้วางออเดอร์ต่ำกว่าราคาปิด 0.4 ATR
            //   แต่เดิมเริ่มเช็ค TP2/SL ทันทีตั้งแต่แท่งที่เกิดสัญญาณ โดยไม่เคยเช็คว่า
            //   ราคาย่อลงมาแตะราคาเข้าหรือยัง ราคาวิ่งขึ้นไปแตะ TP2 โดยไม่เคยย่อลงมา
            ...
            if not tr.pending and bar_index >= tr.barFilled
...
// MTF INFO TABLE
// ══════════════════════════════════════════
// FIX 2 (ต่อ) — TF ที่ต่ำกว่ากราฟ แสดง n/a ให้เห็นชัด ไม่ใช่โชว์ "-" ปนกับค่าจริง
...
if barstate.islast
    ...
    // FIX 2 (ต่อ) — เกณฑ์เดิม mtfSum >= 3 สมมติว่ามี 5 TF เสมอ ผิดทันทีเมื่อบาง TF ถูกตัด
