...
minLotOz   = input.float(1.0, "ขนาดเล็กสุดที่โบรกรับ (oz)", minval=0.01, step=0.01, group=grp_micro, tooltip="ใช้กันไม่ให้ backtest แบ่งไม้เล็กกว่าที่เทรดจริงได้ — 0.01 lot = 1 oz / บัญชี cent ใส่ 0.01")
...
// ปิดครึ่งไม้ทำได้ต่อเมื่อครึ่งหนึ่งยังไม่เล็กกว่าขนาดที่โบรกรับ
// (ถ้าไม่กันตรงนี้ backtest จะแบ่ง 1 oz เป็น 0.5 oz ซึ่งโบรกจริงทำไม่ได้)
partQty    = posOz / 2
canPartial = usePartial and partQty >= minLotOz
...
if barstate.islast
    ...
    table.cell(t, 1, 12, "$" + str.tostring(acctSize, "#.##") + "  /  " + str.tostring(posOz, "#.##") + " oz" + (canPartial ? "  (แบ่งครึ่งได้)" : "  (TP2 อย่างเดียว)"), text_color=color.white, text_size=size.tiny, bgcolor=color.new(#0f3460, 40))
