# XAUUSD SMC Pro v9.2 STRATEGY — โค้ดเต็มสำหรับคัดลอก

แก้ 3 บั๊กของ v9 + เพิ่ม break-even ทุกอย่างเป็นสวิตช์ ปิดหมด = ได้ v9 เดิม
แผนการทดสอบอยู่ใน [TEST_PLAN_v9_2.md](TEST_PLAN_v9_2.md)
...
//@version=5
// ══════════════════════════════════════════════════════════════════════════
// XAUUSD SMC Pro v9.2 — STRATEGY (แก้บั๊ก + BE)
//
// ต่อยอดจาก v9 STRATEGY โดยตรง — logic เข้าไม้ ค่า default และสูตร TP/SL
// เหมือน v9 ทุกบรรทัด สิ่งที่เพิ่มคือการแก้ 3 บั๊ก + break-even
// ทุกอย่างเป็นสวิตช์ เปิด/ปิดได้ทีละตัว
//
// ── วิธีทำให้เหมือน v9 เดิมเป๊ะ ๆ (ใช้เป็น baseline) ──
//   กลุ่ม "0. การแก้ของ v9.2" → ปิดให้หมดทั้ง 3 ตัว
//   limitExpiry = 999
//   noRepaint   = ปิด
//
// ── ค่าที่ควรใส่ในแท็บ Properties ตอนทดสอบต้นทุนจริง ──
//   Commission : 0.30  Cash per contract   (spread ทอง ~30 point ต่อ oz)
//   Slippage   : 2 ticks
//   Use bar magnifier : เปิด (ถ้าแพ็กเกจรองรับ)
//   สคริปต์ตั้ง commission = 0 ไว้เท่ากับ v9 เพื่อให้เทียบกันได้ยุติธรรม
//   ค่าในแท็บ Properties จะ override ค่าในโค้ด
// ══════════════════════════════════════════════════════════════════════════
strategy("XAUUSD SMC Pro v9.2 [STRATEGY]", overlay=true,
...
// ══════════════════════════════════════════
// 0. การแก้ของ v9.2 — ปิดทั้งหมด = ได้ v9 เดิม
// ══════════════════════════════════════════
...
useBE      = input.bool(true, "เพิ่ม: เลื่อน SL มา BE หลังโดน TP1", group=grp_fix, tooltip="จาก backtest v9: 51% ของไม้แตะ TP1 แล้วย้อนกลับมาโดน SL จนเหลือสุทธิ $0 — ข้อนี้แก้ตรงจุดนั้น")
...
if buySignal
    ...
    actTP2  := finalBuyTP2
    pendBar := bar_index
    strategy.entry("L", strategy.long, limit = useLimit ? buyEntry : na)
    // ── บั๊ก 2: ติดอาวุธ exit พร้อมกับ entry เลย
    //    v9 รอจนแท่งถัดไปปิดถึงจะเห็นว่ามีโพสิชัน = แท่งที่ limit ติดไม่มี SL
    if fixArmExit
        if usePartial
            strategy.exit("bTP1", from_entry="L", qty=1, limit=actTP1, stop=actSL)
...
if sellSignal
    ...
    actTP2  := finalSellTP2
    pendBar := bar_index
    strategy.entry("S", strategy.short, limit = useLimit ? sellEntry : na)
    if fixArmExit
        if usePartial
            strategy.exit("sTP1", from_entry="S", qty=1, limit=actTP1, stop=actSL)
...
if strategy.position_size > 0
    ...
    if usePartial and not tp1Done
        strategy.exit("bTP1", from_entry="L", qty=1, limit=actTP1, stop=slNow)
...
if strategy.position_size < 0
    ...
    if usePartial and not tp1Done
        strategy.exit("sTP1", from_entry="S", qty=1, limit=actTP1, stop=slNowS)
...
var table t = table.new(position.top_right, 2, 12, bgcolor=color.new(#1a1a2e, 10), border_width=1, border_color=color.new(color.gray, 70))
if barstate.islast
    table.cell(t, 0, 0, "SMC v9.2", text_color=color.white, text_size=size.tiny, bgcolor=color.new(#0f3460, 20))
    table.cell(t, 1, 0, "STRATEGY", text_color=color.white, text_size=size.tiny, bgcolor=color.new(#0f3460, 20))
